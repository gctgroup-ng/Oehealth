...
class intarvasCentersOperatingRooms(models.Model):
    ...
    def unlink(self):
        ...
        return super(intarvasCentersOperatingRooms, self).unlink()
...
# Health Center inheritance to add OT relationship
class intarvasCenters(models.Model):
    _inherit = 'oeh.medical.health.center'

    # One2many relationship for Operation Theaters
    ot_ids = fields.One2many('oeh.medical.health.center.ot', 'institution', string='Operation Theaters')
...
class intarvasCentersBuilding(models.Model):
    ...
    def _ot_count(self):
        for building in self:
            building.ot_count = self.env['oeh.medical.health.center.ot'].sudo().search_count(
