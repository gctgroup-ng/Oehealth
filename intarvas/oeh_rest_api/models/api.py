...
fcm_project_api_key = 'intarvas.fcm_project_api_key'
patient_notification_message_ref = 'intarvas.patient_notification_message'
physician_notification_message_ref = 'intarvas.physician_notification_message'
...
class intarvasRESTAPIToken(models.Model):
    _name = 'oeh.rest.api.token'
    _description = 'Rest API access token'
    _rec_name = 'Token'

    Token = fields.Char('Token', required=True, default='/', copy=False, tracking=True)
    UserId = fields.Many2one('res.users', string='Generated By', default=lambda self: self.env.uid, copy=False, tracking=True)
    ExpirationDate = fields.Datetime('Expires', copy=False, tracking=True)
    TokenGenerated = fields.Boolean(default=False, copy=False, tracking=True)
    ...
    def create(self, vals):
        ...
        return super(intarvasRESTAPIToken, self).create(vals)
...
class intarvasAppointmentReminders(models.Model):
    _name = 'oeh.medical.appointment.notifications'
    _description = 'intarvas - Appointment Reminders'

    name = fields.Integer(string='Appointment ID', tracking=True)
    reminder_notification_sent = fields.Boolean(default=False, tracking=True)
    reminder_notification_date = fields.Datetime(string='Reminder Notification Date & Time', tracking=True)


class intarvasNotifications(models.Model):
    _name = 'oeh.medical.notifications'
    _description = 'intarvas - Push Notifications'
    ...
    patient_id = fields.Integer(string='Patient ID', tracking=True)
    physician_id = fields.Integer(string='Physician ID', tracking=True)
    device_registration_token = fields.Char(string='Device Registration Token', tracking=True)
    device_type = fields.Selection(DEVICE_TYPE, string='Device Type', tracking=True)
    notification_setting = fields.Boolean(string='Notification On?', default=True, tracking=True)
                    ...
                    if appointments:
                        for appointment in appointments:
                            ...
                            if appointment.patient.oeh_patient_user_id:
                                patient_difference = appointment.appointment_date - current_date
                                patient_meeting_time = str(int(patient_difference.seconds / 60))

                            if appointment.doctor.oeh_user_id:
                                physician_difference = appointment.appointment_date - current_date
