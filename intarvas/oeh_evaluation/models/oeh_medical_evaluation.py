...
class intarvasPatientEvaluation(models.Model):
    ...
    name = fields.Char(string='Evaluation #', size=64, readonly=True, required=True, default=lambda *a: '/', tracking=True)
    patient = fields.Many2one('oeh.medical.patient', string='Patient', help="Patient Name", required=True, tracking=True)
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', help="Current primary care / family doctor", domain=[('is_pharmacist','=',False)], required=True, default=_get_physician, tracking=True)
    appointment = fields.Many2one('oeh.medical.appointment', string='Appointment #', tracking=True)
    evaluation_start_date = fields.Datetime(string='Evalution Date', required=True, tracking=True)
    evaluation_end_date = fields.Datetime(string='Evalution End Date', tracking=True)
    derived_from = fields.Many2one('oeh.medical.physician', string='Doctor who escalated the case', tracking=True)
    derived_to = fields.Many2one('oeh.medical.physician', string='Doctor to whom escalated', tracking=True)
    evaluation_type = fields.Selection(EVALUATION_TYPE, string='Evaluation Type', required=True, index=True, default=lambda *a: 'Pre-arranged Appointment', tracking=True)
    chief_complaint = fields.Char(string='Chief Complaint', size=128, help='Chief Complaint', tracking=True)
    notes_complaint = fields.Text(string='Complaint details', tracking=True)
    ...
    info_diagnosis = fields.Text(string='Presumptive Diagnosis', tracking=True)
    directions = fields.Text(string='Plan', tracking=True)
    ...
    def create(self,vals):
        ...
        return super(intarvasPatientEvaluation, self).create(vals)
...
class intarvasPatient(models.Model):
    ...
    evaluation_ids = fields.One2many('oeh.medical.evaluation', 'patient', string='Evaluation', tracking=True)
...
class intarvasAppointment(models.Model):
    ...
    evaluation_ids = fields.One2many('oeh.medical.evaluation', 'appointment', string='Evaluation', readonly=False, tracking=True)
