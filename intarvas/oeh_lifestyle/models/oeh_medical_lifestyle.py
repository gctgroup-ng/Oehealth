...
class intarvasRecreationalDrug(models.Model):
    ...
    name = fields.Char(string='Drug Name', size=128, required=True, tracking=True)
    street_name = fields.Char(string='Street Names', size=256, help="Common name of the drug in street jargon", tracking=True)
    toxicity = fields.Selection(TOXICITY, string='Toxicity', required=True, tracking=True)
    addiction_level = fields.Selection(ADDICTION_LEVEL, string='Addiction Level', required=True, tracking=True)
    legal_status = fields.Selection(LEGAL, string='Legal Status', required=True, tracking=True)
    category = fields.Selection(DRUG_CATEGORY, 'Category', required=True, tracking=True)
    ...
    info = fields.Text(string='Extra Info', tracking=True)
...
class intarvasPatient(models.Model):
    ...
    sexual_preferences = fields.Selection(SEXUAL_PREFERENCE, string='Sexual Orientation', tracking=True)
    sexual_practices = fields.Selection(SEXUAL_PRACTICES, string='Sexual Practices', tracking=True)
    sexual_partners = fields.Selection(SEXUAL_PARTNERS, string='Sexual Partners', tracking=True)
    sexual_partners_number = fields.Integer(string='Number of Sexual Partners', tracking=True)
    first_sexual_encounter = fields.Integer(string='Age first Sexual Encounter', tracking=True)
    anticonceptive = fields.Selection(ANTI_CONCEPTIVE, string='Anticonceptive Method', tracking=True)
    sex_oral = fields.Selection(SEXUAL_PARTNERS, string='Oral Sex', tracking=True)
    sex_anal = fields.Selection(SEXUAL_PARTNERS, string='Anal Sex', tracking=True)
    prostitute = fields.Boolean(string='Prostitute', help="Check if the patient (he or she) is a prostitute", tracking=True)
    sex_with_prostitutes = fields.Boolean(string='Sex with Prostitutes', help="Check if the patient (he or she) has sex with prostitutes", tracking=True)
    sexuality_info = fields.Text(string='Sexuality Information', tracking=True)
