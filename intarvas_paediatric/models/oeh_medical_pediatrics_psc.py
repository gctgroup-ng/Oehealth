...
class intarvasPediatricSymptomsChecklist(models.Model):
    ...
    name = fields.Char(string='PSC #', size=64, readonly=True, required=True, default=lambda *a: '/', tracking=True)
    patient = fields.Many2one('oeh.medical.pediatrics.newborn', string='Patient', help="Patient Name", required=True, tracking=True)
    doctor = fields.Many2one('oeh.medical.physician', string='Doctor', domain=[('is_pharmacist','=',False)], help="Current primary care / family doctor", required=True, tracking=True)
    evaluation_start = fields.Datetime(string='Date', required=True, tracking=True)
    ...
    psc_total = fields.Integer(string='PSC Total', required=True, default=lambda *a: 0, tracking=True)

    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('oeh.medical.pediatrics.psc')
        vals['name'] = sequence
        return super(intarvasPediatricSymptomsChecklist, self).create(vals)
